...
// the agent has no LiveKit video-track avatar.
const SHOW_DIGITAL_HUMAN = true;
...
export function TileLayout({
  ...
  return (
    <div
      className={cn(
        'absolute top-8 z-50 md:top-12',
        // When chat is open, the avatar keeps its size and moves to the left
        // half (top half on mobile); the transcript fills the other half.
        chatOpen
          ? 'inset-x-0 bottom-1/2 md:right-1/2 md:bottom-40'
          : 'inset-x-0 bottom-32 md:bottom-40'
      )}
    >
        ...
        <div className={cn(tileViewClassNames.grid)}>
          ...
          >
            <AnimatePresence mode="popLayout">
              {!isAvatar && SHOW_DIGITAL_HUMAN && (
                // Digital human (FaceUnity 3D avatar)
                <motion.div
                  key="digital-human"
                  layoutId="agent"
                  initial={{ opacity: 0 }}
