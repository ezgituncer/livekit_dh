...
# Qwen-Omni-Realtime (DashScope) defaults. The Qwen endpoint speaks the legacy
# (beta) OpenAI realtime wire format and only accepts its own voices / ASR model,
# so OpenAI defaults (voice "marin", transcription "gpt-4o-mini-transcribe") and
# the GA-nested session shape ("output_modalities", nested `audio.format`,
# "semantic_vad") are rejected outright — which silently disables all output.
...
# Qwen-Omni-Realtime emits the same legacy (beta) event names as Azure, so the
# normalization is identical.
_QWEN_EVENT_MAPPING: dict[str, str] = dict(_AZURE_EVENT_MAPPING)
...
def _to_qwen_turn_detection(
    turn_detection: RealtimeAudioInputTurnDetection | None,
) -> dict[str, Any] | None:
    """Qwen-Omni-Realtime only supports ``server_vad``. Map whatever turn-detection
    config we have (including OpenAI's default ``semantic_vad``) onto it."""
    if turn_detection is None:
        return None

    if isinstance(turn_detection, BaseModel):
        d = turn_detection.model_dump(exclude_none=True)
    else:
        d = dict(turn_detection)

    out: dict[str, Any] = {"type": "server_vad"}
    if d.get("type") == "server_vad":
        for key in ("threshold", "prefix_padding_ms", "silence_duration_ms"):
            if d.get(key) is not None:
                out[key] = d[key]
    else:
        # semantic_vad (or anything else) — fall back to Qwen server_vad defaults.
        # A slightly higher threshold than the 0.5 baseline makes the VAD less
        # likely to trigger on background noise (which the ASR otherwise transcribes
        # as Chinese filler like "嗯").
        out["threshold"] = 0.6
        out["silence_duration_ms"] = 800
    return out


def _oai_session_to_qwen(session: RealtimeSessionCreateRequest) -> dict[str, Any]:
    """Convert a GA-style ``RealtimeSessionCreateRequest`` into Qwen-Omni-Realtime's
    flat (legacy) session shape.

    Qwen does not understand ``output_modalities``, the nested ``audio`` block, the
    ``audio/pcm`` format object, or ``semantic_vad``. It expects flat top-level
    fields: ``modalities``, ``voice``, ``input_audio_format``/``output_audio_format``
    (the string ``"pcm"``), ``input_audio_transcription``, and ``turn_detection``.
    Sending the GA shape makes Qwen drop the session config, which disables all
    text/audio output. Only the fields actually set on ``session`` are emitted so
    this works for both full-session creation and incremental ``update_options``.
    """
    mapped: dict[str, Any] = {}

    if session.output_modalities is not None:
        # Qwen requires ["text", "audio"] for audio (audio-only is rejected); it
        # replies with audio + a text transcript.
        mapped["modalities"] = (
            ["text", "audio"] if "audio" in session.output_modalities else ["text"]
        )
        mapped["input_audio_format"] = "pcm"
        mapped["output_audio_format"] = "pcm"
    ...
    if session.audio is not None:
        ...
        if inp is not None:
            if inp.transcription is not None:
                model = getattr(inp.transcription, "model", None)
                transcription: dict[str, Any] = {
                    "model": model or QWEN_DEFAULT_TRANSCRIPTION_MODEL
                }
                # Pass a language hint so the ASR doesn't auto-detect and fall back
                # to Chinese filler ("嗯") on noise/silence.
                lang = getattr(inp.transcription, "language", None)
                if lang:
                    transcription["language"] = lang
                mapped["input_audio_transcription"] = transcription
            if inp.turn_detection is not None:
                mapped["turn_detection"] = _to_qwen_turn_detection(inp.turn_detection)
        if out is not None and out.voice is not None:
            mapped["voice"] = out.voice
    ...
    if session.instructions is not None:
        mapped["instructions"] = session.instructions
    # Emit "tools" whenever it's set at all (even an empty list) — update_tools()
    # indexes ev["session"]["tools"] unconditionally, so dropping the key on an
    # empty tool list raises KeyError and crashes the session at startup.
    if session.tools is not None:
        mapped["tools"] = [
            t.model_dump(by_alias=True, exclude_unset=True, exclude_none=True)
    ...
    if session.tool_choice is not None:
        mapped["tool_choice"] = (
            session.tool_choice.model_dump()
            if isinstance(session.tool_choice, BaseModel)
...
class RealtimeModel(llm.RealtimeModel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Detect Qwen-Omni-Realtime (DashScope). It speaks the legacy realtime wire
        # format and rejects OpenAI-specific defaults, so we substitute Qwen-valid
        # defaults for the voice and transcription model when the caller didn't set
        # them explicitly.
        is_qwen = "dashscope" in base_url_val.lower() or "qwen" in model.lower()
        if is_qwen:
            if voice == DEFAULT_VOICE:
                voice = QWEN_DEFAULT_VOICE
            if not is_given(input_audio_transcription):
                input_audio_transcription = AudioTranscription(
                    model=QWEN_DEFAULT_TRANSCRIPTION_MODEL
                )
...
class RealtimeSession(
    ...
    async def _run_ws(self, ws_conn: aiohttp.ClientWebSocketResponse) -> None:
        ...
        async def _send_task() -> None:
            ...
            async for msg in self._msg_ch:
                try:
                    ...
                    # Legacy Azure and Qwen both use "text" for assistant content
                    # parts, while the new API uses "output_text".
                    if (self._opts.is_azure and self._opts.api_version) or self._opts.is_qwen:
        ...
        async def _recv_task() -> None:
            while True:
                ...
                # Both Qwen and Azure emit legacy (beta) event names; normalize them
                # to the GA names the handlers below expect.
                if self._realtime_model._opts.is_qwen:
                    normalized = _QWEN_EVENT_MAPPING.get(event.get("type", ""))
                    if normalized is not None:
                        event["type"] = normalized

                if self._realtime_model._opts.is_azure:
                    normalized = _AZURE_EVENT_MAPPING.get(event.get("type", ""))
                    if normalized is not None:
                        event["type"] = normalized
    ...
    def _wrap_session_update(
        ...
        """
        if self._opts.is_qwen:
            # Qwen-Omni-Realtime: emit the flat legacy session shape as a plain dict.
            return {
                "type": "session.update",
                "event_id": event_id,
                "session": _oai_session_to_qwen(session),
            }
    ...
    async def update_chat_ctx(self, chat_ctx: llm.ChatContext) -> None:
        async with self._update_chat_ctx_lock:
            ...
            if self._opts.is_qwen:
                # Qwen-Omni-Realtime doesn't reliably echo `conversation.item.created`
                # with our item ids, so blocking on those acks just times out and
                # crashes the whole session (RealtimeError). Events are sent FIFO over
                # the same socket, so the item still reaches Qwen before the following
                # response.create — the ack was only used to sync remote-ctx ids.
                # Send fire-and-forget and optimistically mirror the change into the
                # remote ctx so the next diff converges (no re-send churn).
                for ev in events:
                    self.send_event(ev)
                    self._apply_remote_ctx_optimistically(ev)
                return
    ...
    def _apply_remote_ctx_optimistically(
        self, ev: ConversationItemCreateEvent | ConversationItemDeleteEvent
    ) -> None:
        """Mirror a create/delete event into the remote chat ctx without waiting for a
        server ack (used for Qwen, which doesn't reliably ack with our ids). Best
        effort: a later real ack hitting the same id is already handled gracefully in
        `_handle_conversion_item_added`."""
        try:
            if isinstance(ev, ConversationItemDeleteEvent):
                self._remote_chat_ctx.delete(ev.item_id)
            elif isinstance(ev, ConversationItemCreateEvent) and ev.item.id is not None:
                prev = None if ev.previous_item_id in (None, "root") else ev.previous_item_id
                self._remote_chat_ctx.insert(prev, openai_item_to_livekit_item(ev.item))
        except Exception as e:  # noqa: BLE001 — purely best-effort bookkeeping
            logger.debug("optimistic remote ctx update skipped: %s", e)
    ...
    def generate_reply(
        ...
        *,
        ...
        # Qwen-Omni-Realtime uses the legacy response.create shape and neither
        # accepts a `metadata` field nor echoes it back on `response.created`, so we
        # can't correlate the reply via metadata. Keep the event minimal (only set
        # fields that were explicitly given) and instead resolve the pending future
        # FIFO in `_handle_response_created` (see there).
        if self._opts.is_qwen:
    ...
    def _handle_response_created(self, event: ResponseCreatedEvent) -> None:
        ...
        elif self._opts.is_qwen and self._response_created_futures:
            # Qwen doesn't round-trip our client_event_id, so a response.created with
            # no metadata that arrives while a generate_reply (e.g. typed text) is
            # pending must be that reply. Correlate FIFO.
            oldest_key = next(iter(self._response_created_futures))
            fut = self._response_created_futures.pop(oldest_key, None)
